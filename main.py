...
display(f'Countries I want to travel to: {fav_country101_[0], fav_country101_[1], fav_country101_[2]}', target="display")
display(f'Student Type: {student_type}', target="display")
display(f'Favorite Color, Car Brand, Shoe Size, and Best Friend: ',  descriptions["color"], descriptions["car_brand"], descriptions["shoe_size"], descriptions["best_friend"], target="display")
...
def floor_divide(e):
    ...
    num2= float(document.getElementById("input2").value)
    result= num1 // num2
    display(f'Your quotient (rounded rown) is equal to {result}.', target="output")
